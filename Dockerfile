...
# Cài đặt các công cụ build cần thiết (nếu dependencies của bạn cần biên dịch native)
RUN apk add --no-cache python3 make g++

WORKDIR /SmartDebt_BackEnd

# Copy file cấu hình trước để tận dụng Docker layer caching
COPY package*.json ./
...
# Nếu dự án của bạn dùng TypeScript hoặc cần build (ví dụ: NestJS), hãy uncomment dòng dưới
# RUN npm run build
...
# Azure Web App thường mặc định lắng nghe port 3000 cho container
EXPOSE 3000
